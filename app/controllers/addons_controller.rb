class AddonsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @addon.save
        format.html { redirect_to addon_url(@addon), notice: "Addon was successfully created." }
  ...
  def update
    respond_to do |format|
      if @addon.update(addon_params)
        format.html { redirect_to addon_url(@addon), notice: "Addon was successfully updated." }
  ...
  private
    ...
    def addon_params
      params.require(:addon).permit(:name, :price, :description, :image_url)
    end
end
